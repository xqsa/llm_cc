...
Current repository state: `Stage 8.18 PASS` — Stage 8.18 reruns a bounded CEC2013 F13/F14 single-seed smoke with the Stage 8.16 repaired policy `reward_trust_gated_coordination_v1` before any full 25-run panel. It records function_ids = F13/F14, run_count = 1, smoke_max_fe_per_method_per_function = 1200, method_count = 6, trace_row_count = 14400, FE_total = 28812, repaired_vs_stage8_11_generalized_policy = 2 win / 0 tie / 0 loss, repaired_vs_best_reward_select = 0 win / 2 tie / 0 loss, repaired_vs_best_baseline = 0 win / 2 tie / 0 loss, repaired_policy_resmoke_promising = true, and routes to Stage 8.19 CEC2013 F13/F14 repaired-policy multiseed pilot. It is not a full 25-run panel, not a full F1-F15 panel, not an official CEC2013 benchmark success, not a final objective-value performance claim, and not a SOTA claim.
...
```text
Stage 8.19: CEC2013 F13/F14 repaired-policy multiseed pilot
```

Stage 8.19 should run a bounded CEC2013 F13/F14 multiseed pilot with `reward_trust_gated_coordination_v1` before spending the full CEC2013 25-run budget. It must keep BaseOpt fixed, avoid validation/test feedback leakage, preserve FE accounting, compare against best_reward_select and Stage 8.11 generalized policy, avoid reported-results runtime feedback, check whether Stage 8.18's no-loss result is stable across seeds, and remain not a final objective-value performance claim and not a SOTA claim.

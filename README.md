...
Current repository state: `Stage 8.32 PASS` — Stage 8.32 designs `stage8_32_guarded_owner_trust_repair_v1`, a no-objective, no-LLM, no-CEC repair-design gate for the Stage 8.31 overcorrection diagnosis. It changes the policy design from always `contribution_leader + break` to a guarded policy: if `best_reward_reliable`, preserve `trust_best_reward / preserve / best_reward_group`; only if `strong_owner_conflict AND best_reward_misleading`, use `contribution_leader + break` with `owner_proposal_select`; if unstable or uncertain, preserve with shrinkage/weighted safety; otherwise preserve weighted/simple safety. It records overcorrection_guard_designed = true, all_required_guard_paths_covered = true, FE_total = 0, objective_loop_executed = false, new_objective_evaluation_used = false, cec_checkpoint_executed = false, llm_call_used = false, new_llm_strategy_generation_used = false, selected_policy_revision_used = false, formal_25_run_recommended_now = false, and routes to Stage 8.33 static guard sanity or bounded checkpoint gate before any formal panel. It is not a final objective-value performance claim and not a SOTA claim.
...
Stage 8.33 should check the Stage 8.32 guarded repair design with a small sanity or bounded checkpoint gate before any formal 25-run panel. It should verify that the guard does not collapse back to always `contribution_leader + break`, preserves best-reward trust when reliable, and only then decide whether a CEC checkpoint is justified. It should not make a SOTA claim.

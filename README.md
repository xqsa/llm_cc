...
Current repository state: `Stage 8.19 PASS` — Stage 8.19 is a design-lock stage that redirects the next research route from static one-shot LLM candidate generation to evaluator-grounded LLM-reflective shared-variable coordination policy search. It records stage8_18_policy_equivalent_to_best_reward = true, non_trust_best_reward_branch_exercised_on_stage8_18 = false, static_one_shot_llm_candidate_generation_rejected = true, fake_llm_candidates_forbidden = true, objective_evaluator_feedback_required = true, llm_contribution_ablation_required = true, beat_best_reward_required = true, FE_total = 0, and routes to Stage 8.20 execute LLM-reflective coordination policy search. It is not an LLM execution stage, not a new objective run, not a final objective-value performance claim, and not a SOTA claim.
...
```text
Stage 8.20: execute LLM-reflective coordination policy search
```

Stage 8.20 should execute the LLM-reflective shared-variable coordination policy search loop locked by Stage 8.19. It must use real LLM API calls rather than fake candidates, mine Stage 8.18 best_reward degeneracy patterns, generate typed coordination policy programs under the DSL boundary, evaluate candidates on train-side objective cases, feed evaluator summaries back to the LLM for at least two reflection rounds, compare the LLM-reflective pool against non-LLM pools, and require at least one non-degenerate LLM-origin candidate that beats `best_reward_select` without validation/test feedback leakage. It remains not a final objective-value performance claim and not a SOTA claim.

...
Current repository state: `Stage 8.17 PASS` — Stage 8.17 executes the Stage 8.16 repaired policy `reward_trust_gated_coordination_v1` in a bounded train-side objective loop before any full CEC2013 panel. It records case_count = 8, method_count = 6, trace_row_count = 192, FE_total = 384, all repaired branches exercised, repaired_vs_stage8_11_generalized_policy = 5 win / 3 tie / 0 loss, repaired_vs_best_reward_select = 6 win / 2 tie / 0 loss, repaired_vs_best_baseline = 2 win / 6 tie / 0 loss, bounded_check_promising = true, and routes to Stage 8.18 CEC2013 F13/F14 repaired-policy re-smoke. It is not a full 25-run panel, not an official CEC2013 benchmark success, not a final objective-value performance claim, and not a SOTA claim.
...
Stage 8.18 should re-run a bounded CEC2013 F13/F14 single-seed smoke with `reward_trust_gated_coordination_v1` before spending the full CEC2013 25-run budget. It must keep BaseOpt fixed, avoid validation/test feedback leakage, preserve FE accounting, compare against best_reward_select and Stage 8.11 generalized policy, avoid reported-results runtime feedback, and remain not a final objective-value performance claim and not a SOTA claim.

...
This pilot embeds selection-ready operators into a fixed objective-level
LOCO-CC loop and compares their utility against the frozen Stage 5.1 selected
operator. It does not call LLMs, generate new candidates, modify BaseOpt, or
...
STAGE = "8.2"
TRACE_SCHEMA_VERSION = "loco.stage8_2_objective_trace.v1"
SUMMARY_SCHEMA_VERSION = "loco.stage8_2_method_summary.v1"
FE_LEDGER_SCHEMA_VERSION = "loco.stage8_2_fe_ledger.v1"
BOUNDARY_SCHEMA_VERSION = "loco.stage8_2_runtime_boundary.v1"
REPORT_SCHEMA_VERSION = "loco.stage8_2_pilot_report.v1"
UTILITY_TRACE_SCHEMA_VERSION = "loco.stage8_2_utility_trace.v1"
UTILITY_REPORT_SCHEMA_VERSION = "loco.stage8_2_utility_report.v1"
...
def run_stage8_2_objective_level_loco_cc_loop_pilot(
    *,
    ...
    """Run an objective-level LOCO-CC pilot with selection-ready operators."""
    ...
    selection_ready_operator = _build_selection_ready_operator(
        selection_ready_candidate, selected_operator, selected_ast_payload
    )
    ...
    selected_variable = int(selected_operator["target_variable_set"][0])
    selection_ready_runtime = FrozenASTRuntime(
        load_coordination_ast(selection_ready_candidate["llm_candidate_payload"]["ast"])
    )
    methods = _build_methods(
        selected_runtime,
        selected_candidate_id,
        selected_variable,
        selection_ready_runtime,
        selection_ready_operator,
        str(selection_ready_candidate["candidate_id"]),
    )
    ...
    for method in methods:
        method_rows = _run_method_loop(
            method=method,
            selected_candidate_id=selected_candidate_id,
            selected_variable=selected_variable,
            pilot_problem=pilot_problem,
        )
        trace_rows.extend(method_rows)
        utility_rows.append(
            _utility_row(method_rows=method_rows, method_name=method.name)
        )
    ...
    utility_report = _build_utility_report(
        utility_rows, str(selection_ready_candidate["candidate_id"])
    )
    report = _build_report(
        ...
        selection_ready_candidate_id=str(selection_ready_candidate["candidate_id"]),
    )
    route = _build_route()
    ...
    _write_jsonl(output_path / "utility_trace.jsonl", utility_rows)
    _write_json(output_path / "utility_report.json", utility_report)
...
def _validate_inputs(
    ...
    if protocol.get("stage") != "7.0":
        raise ValueError("Stage 8.2 requires the Stage 7.0 protocol.")
    if protocol.get("next_status") != "READY_FOR_STAGE7_1_MINIMAL_OBJECTIVE_LOOP_PILOT":
        raise ValueError("Stage 7.0 protocol is not ready for objective-loop use.")
    if selection_decision.get("stage") != "8.1":
        raise ValueError("Stage 8.2 requires the Stage 8.1 selection decision.")
    ...
    if selected_operator.get("stage") != "5.1":
        raise ValueError("Stage 8.2 requires the Stage 5.1 selected operator.")
    if selected_operator.get("freeze_status") != "FROZEN_FOR_SEALED_TEST_NOT_FINAL":
        raise ValueError("Stage 8.2 requires the frozen Stage 5.1 operator.")
    ...
    if selected_operator.get("target_scope") != "shared_variables_only":
        raise ValueError("Stage 8.2 requires a shared-variable operator.")
...
def _choose_selection_ready_candidate(
    selection_decision: Mapping[str, Any],
    *,
    selected_variable: int,
) -> Mapping[str, Any]:
    candidates = selection_decision["selection_ready_candidates"]
    if not candidates:
        raise ValueError("Stage 8.2 requires at least one selection-ready candidate.")
    pool_by_id = {
        row["candidate_id"]: row for row in _read_jsonl(FROZEN_CANDIDATE_POOL_PATH)
    }
    for candidate in candidates:
        row = pool_by_id.get(str(candidate["candidate_id"]))
        if row is None:
            continue
        if selected_variable in row.get("target_variable_set", []):
            return row
    raise ValueError(
        "Stage 8.2 requires a selection-ready candidate targeting the selected "
        "shared variable."
    )


def _build_selection_ready_operator(
    candidate_row: Mapping[str, Any],
    frozen_selected_operator: Mapping[str, Any],
    selected_ast_payload: Mapping[str, Any],
) -> dict[str, Any]:
    return {
        "candidate_id": str(candidate_row["candidate_id"]),
        "source_stage": "8.1",
        "source_candidate_pool_stage": "3.6",
        "target_scope": "shared_variables_only",
        "target_variable_set": list(frozen_selected_operator["target_variable_set"]),
        "operator_family": str(candidate_row["operator_family"]),
        "kind_sequence": str(candidate_row["kind_sequence"]),
        "source_ast_operator_id": str(
            candidate_row["llm_candidate_payload"]["ast"]["operator_id"]
        ),
...
def _build_methods(
    ...
    selected_variable: int,
    selection_ready_runtime: FrozenASTRuntime,
    selection_ready_operator: Mapping[str, Any],
    selection_ready_candidate_id: str,
) -> list[_Method]:
    return [
        _Method(
        ...
        _Method(
            name="selection_ready_stage8_operator",
            label=f"SelectionReady({selection_ready_candidate_id})",
            is_selected_loco=True,
            coordinate=lambda state: selection_ready_runtime.coordinate(
                state, shared_variables={selected_variable}
            ),
        ),
    ]
...
def _build_pilot_problem(selected_variable: int) -> dict[str, Any]:
    ...
    metadata = build_overlap_metadata(
        groups,
        dimension=dimension,
        topology="stage8_2_objective_level_pilot",
        grouping_source="oracle grouping",
        grouping_confidence="high",
    )
    return {
        "name": "stage8_2_synthetic_sphere_pilot",
...
def _run_method_loop(
    *,
    ...
    current[selected_variable] = float(pilot_problem["initial_shared_value"])
...
def _utility_row(
    *, method_rows: Sequence[Mapping[str, Any]], method_name: str
) -> dict[str, Any]:
    ...
    return {
        ...
        "objective_final_best": final_best,
        "objective_final_value": baseline_final,
        "improved_or_equal_steps": int(improvement_count),
        "utility_score": round(max(0.0, baseline_final - final_best), 12),
        "objective_level_utility_evaluated": True,
        "not_performance_claim": True,
    }
...
def _build_method_summary(
    ...
    return {
        ...
        "utility_scope": "objective_level_loco_cc_loop_pilot",
        "claim_scope": "objective_level_utility_pilot",
        "not_sota_claim": True,
        "not_final_performance_claim": True,
    }
...
def _build_fe_ledger(trace_rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "stage": STAGE,
        "status": "PASS",
        "budget_scope": "objective_level_loco_cc_loop_pilot",
...
def _build_runtime_boundary() -> dict[str, Any]:
    return {
        ...
        "stage": STAGE,
        "status": "PASS",
        "claim_scope": "objective-level utility pilot",
        "forbidden_behaviors": {
            "llm_call": False,
...
def _build_utility_report(
    ...
    selection_ready_row = next(
        row
        for row in utility_rows
        if row["method_name"] == "selection_ready_stage8_operator"
    )
    selected_loco_rows = [
        frozen_row,
        selection_ready_row,
    ...
    best_row = min(
        selected_loco_rows, key=lambda row: float(row["objective_final_best"])
    )
    best_candidate_rank = selected_loco_rows.index(best_row) + 1
    utility_delta = round(
        float(selection_ready_row["objective_final_best"])
        - float(frozen_row["objective_final_best"]),
        12,
    )
    return {
        "schema_version": UTILITY_REPORT_SCHEMA_VERSION,
        "stage": STAGE,
        "status": "PASS",
        "source_stage": "8.1",
        "baseline_selected_candidate_id": "stage3_5_batch_1_weighted_consensus_projection",
        "selection_ready_candidate_id": selection_ready_candidate_id,
        "utility_ready_candidate_count": len(utility_rows),
        "utility_candidates": list(utility_rows),
        "selected_loco_method_count": len(selected_loco_rows),
        "selected_loco_methods": [row["method_name"] for row in selected_loco_rows],
        "best_candidate_rank": best_candidate_rank,
        "frozen_selected_operator_final_best": float(
            frozen_row["objective_final_best"]
        ),
        "selection_ready_candidate_final_best": float(
            selection_ready_row["objective_final_best"]
        ),
        "selection_ready_vs_frozen_selected_operator_delta": utility_delta,
        "selection_ready_improved_over_frozen_selected_operator": utility_delta < 0.0,
        "best_candidate_improved_or_equal_methods": sum(
            1 for row in utility_rows if row["improved_or_equal_steps"] >= 1
        ),
        "selection_ready_candidates_used": True,
        "objective_level_utility_evaluated": True,
        "not_performance_claim": True,
    }
...
def _build_report(
    *,
    ...
    return {
        ...
        "utility_trace_row_count": len(utility_report["utility_candidates"]),
...
def _build_route() -> dict[str, Any]:
    return {
        "schema_version": "loco.stage8_2_next_route_decision.v1",
        "stage": STAGE,
        "status": "PASS",
        "decision": "LOCK_OBJECTIVE_LEVEL_UTILITY_PILOT",
        "decision_reason": (
            "Selection-ready operators were exercised in a fixed objective-level "
            "LOCO-CC loop and compared against the frozen Stage 5.1 operator."
        ),
        "next_stage": "Stage 8.3",
        "allowed_next_work": "train_only_or_validation_selection",

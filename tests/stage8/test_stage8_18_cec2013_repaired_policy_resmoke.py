...
ROOT = Path("E:/llm_cc")
CONFIG = ROOT / "configs" / "stage8_18_cec2013_repaired_policy_resmoke.yaml"
STAGE8_17_DIR = ROOT / "artifacts" / "objective_eval" / "stage8_17"
OUTPUT_DIR = ROOT / "artifacts" / "objective_eval" / "stage8_18"
RESMOKE_REPORT = OUTPUT_DIR / "repaired_policy_resmoke_report.json"
...
STAGE_DOC = ROOT / "docs" / "stage8" / "stage8_18_cec2013_repaired_policy_resmoke.md"
SELF_CHECK = ROOT / "docs" / "stage8" / "stage8_18_self_check_report.md"
...
def test_stage8_18_runs_cec2013_repaired_policy_resmoke(tmp_path) -> None:
    from loco.coordination.cec2013_repaired_policy_resmoke import (
        run_stage8_18_cec2013_repaired_policy_resmoke,
    )

    report = run_stage8_18_cec2013_repaired_policy_resmoke(
    ...
    assert report["stage"] == "8.18"
    assert report["status"] == "PASS"
    assert report["source_stage"] == "8.17"
    assert report["resmoke_scope"] == "cec2013_f13_f14_repaired_policy_resmoke"
    ...
    assert report["trace_row_count"] == 144
    ...
    assert report["repaired_vs_best_reward_select"]["loss"] == 0
    assert report["repaired_policy_resmoke_promising"] is True
    assert report["run_full_25_run_panel_next"] is False
    assert report["recommended_next_stage"] == "Stage 8.19"
    assert report["recommended_next_work"] == "cec2013_f13_f14_repaired_policy_multiseed_pilot"
    ...
    assert "stage8_16_reward_trust_gated_policy" in method_summary["methods"]
    assert win_loss["repaired_vs_stage8_11_generalized_policy"]["loss"] == 0
    assert win_loss["repaired_vs_best_reward_select"]["loss"] == 0
    assert branch["policy_trace_row_count"] == 24
    assert branch["branch_counts"]["trust_best_reward"] >= 1
    assert ledger["FE_total"] == report["FE_total"]
    assert ledger["smoke_max_fe_per_method_per_function"] == 12
    assert ledger["official_cec2013_panel_run"] is False
    assert boundary["not_full_25_run_panel"] is True
    assert route["next_stage"] == "Stage 8.19"
    assert route["run_full_25_run_panel_next"] is False


def test_stage8_18_committed_artifacts_docs_and_readme_record_resmoke() -> None:
    required = [
        CONFIG,
        RESMOKE_REPORT,
        TRACE,
    ...
    report = json.loads(RESMOKE_REPORT.read_text(encoding="utf-8"))

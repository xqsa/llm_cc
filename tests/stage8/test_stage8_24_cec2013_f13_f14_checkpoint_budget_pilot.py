...
CONFIG = ROOT / "configs" / "stage8_24_cec2013_f13_f14_checkpoint_budget_pilot.yaml"
...
STAGE_DOC = ROOT / "docs" / "stage8" / "stage8_24_cec2013_f13_f14_checkpoint_budget_pilot.md"
SELF_CHECK = ROOT / "docs" / "stage8" / "stage8_24_self_check_report.md"
...
def test_stage8_24_runs_frozen_policy_checkpoint_budget_pilot(tmp_path) -> None:
    from loco.coordination.cec2013_frozen_policy_checkpoint_pilot import (
        DEFAULT_CHECKPOINT_MAX_FE,
        run_stage8_24_cec2013_f13_f14_checkpoint_budget_pilot,
    )

    assert DEFAULT_CHECKPOINT_MAX_FE == 120000

    report = run_stage8_24_cec2013_f13_f14_checkpoint_budget_pilot(
    ...
    assert report["checkpoint_budget_pilot_executed"] is True
    assert report["multiseed_pilot_executed"] is False
    ...
    assert report["recommended_next_stage"] in {"Stage 8.25", "Stage 8.24"}
    ...
    assert boundary["claim_scope"] == "CEC2013 F13/F14 frozen-policy checkpoint-budget pilot only"
    assert boundary["not_full_25_run_panel"] is True
    assert route["run_full_25_run_panel_next"] is False


def test_stage8_24_committed_artifacts_docs_and_readme_record_checkpoint_pilot() -> None:
    required = [
        CONFIG,
        REPORT,
        CHECKPOINT_TRACE,
    ...
    assert report["selected_candidate_id"] == "stage8_20_round_candidate_8"
    assert report["frozen_policy_used"] is True
    assert report["max_fe_per_method_per_function"] == 120000
    assert report["run_count"] == 3
    assert report["checkpoint_budget_pilot_executed"] is True
    assert report["not_full_25_run_panel"] is True
    assert report["full_objective_trace_written"] is False
    ...
    assert ledger["FE_total"] == report["FE_total"]
    assert route["next_stage"] in {"Stage 8.25", "Stage 8.24"}

    combined = "\n".join(
        path.read_text(encoding="utf-8")
        for path in [CONFIG, STAGE_DOC, SELF_CHECK, README]
    )
    assert "Current repository state: `Stage 8.25 PASS`" in combined
